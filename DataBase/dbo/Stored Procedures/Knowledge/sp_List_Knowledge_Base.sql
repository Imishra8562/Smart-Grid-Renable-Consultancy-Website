...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT 
            KB.*
        FROM tbl_Knowledge_Base KB
        WHERE
            (@Knowledge_Base_Id IS NULL 
                OR KB.Knowledge_Base_Id = @Knowledge_Base_Id)
        AND (@Knowledge_Base_Url_Link IS NULL 
                OR KB.Knowledge_Base_Url_Link = @Knowledge_Base_Url_Link)
        AND KB.Is_Active = 1
        ORDER BY KB.Knowledge_Base_Id DESC;

    END TRY
    BEGIN CATCH
        DECLARE @ErrorMessage NVARCHAR(MAX);
        SET @ErrorMessage =
            'SP ERROR : LIST Knowledge Base FAILED'
            + CHAR(13) + CHAR(10)
            + ERROR_MESSAGE();
